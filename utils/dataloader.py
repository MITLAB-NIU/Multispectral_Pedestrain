...
class IanDataset(Dataset):
    def __init__(self, annotation_lines):
        super(IanDataset, self).__init__()
        self.annotation_lines   = annotation_lines
        self.length             = len(self.annotation_lines)
    ...
    def __getitem__(self, index):
        index       = index % self.length
        image, label  = self.get_data(self.annotation_lines[index])
        image       = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
    ...
    def get_data(self, annotation_line):
        ...
        label     = np.array([np.array(list(label for label in line[1:]))])
...
def yolo_dataset_collate(batch):
    ...
    return (A_images, A_bboxes), (B_images, B_bboxes), (C_images, C_bboxes)
